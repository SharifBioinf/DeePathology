...
def contractive_dropout_autoencoder(local_data_folder, local_result_folder, model_specific, seed=2018):
    seed = seed
    ...
    sss = StratifiedShuffleSplit(n_splits=2, test_size=0.10, random_state=2017)

    m_rna_train = np.array([])
    m_rna_test = np.array([])
    mi_rna_train = np.array([])
    mi_rna_test = np.array([])
    categorical_tissue_train = np.array([])
    categorical_tissue_test = np.array([])
    categorical_disease_train = np.array([])
    categorical_disease_test = np.array([])

    for train_index, test_index in sss.split(m_rna, categorical_tissue):
        print("TRAIN:", train_index, "TEST:", test_index)
        np.savetxt(X=train_index, fname=dataset_folder + "train_index_stratified_CV.csv", delimiter=",", fmt='%d')
        np.savetxt(X=test_index, fname=dataset_folder + "test_index_stratified_CV.csv", delimiter=",", fmt='%d')
        m_rna_train, m_rna_test = m_rna[train_index], m_rna[test_index]
        mi_rna_train, mi_rna_test = mi_rna[train_index], mi_rna[test_index]
        categorical_tissue_train, categorical_tissue_test = categorical_tissue[train_index], categorical_tissue[test_index]
        categorical_disease_train, categorical_disease_test = categorical_disease[train_index], categorical_disease[test_index]
    ...
    result_folder = local_result_folder + model_specific + "stratified_CV_"
    ...
    for i in range(1, 51):
        ...
        for _, test_index in sss.split(m_rna, categorical_tissue):
            m_rna_temp_test = m_rna_dropout[test_index]
    ...
    for i in range(1, 26):
        ...
        for _, test_index in sss.split(m_rna, categorical_tissue):
            m_rna_temp_test = m_rna_noisy[test_index]
        score = model.evaluate(m_rna_temp_test, [m_rna_temp_test, mi_rna_test, categorical_tissue_test, categorical_disease_test], verbose=0, batch_size=batch_size)
        print(score)
