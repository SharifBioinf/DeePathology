...
def contractive_autoencoder(local_data_folder, local_result_folder, model_specific, seed=2018):
    seed = seed
    ...
    def create_model():
        ...
        decoded_micro_rna = Dense(units=mi_rna.shape[1], activation='linear', name="mi_rna")(inputs_5)
    ...
    model.save(filepath=result_folder + "scae.h5")
    layer_name = "encoded"
    ...
    np.savetxt(X=encoded_output, fname=result_folder + "encoded_scae.csv", delimiter=",")

    # save the result and prediction value

    data_pred = model.predict(m_rna_test, batch_size=batch_size, verbose=2)
    np.savetxt(X=m_rna_test, fname=result_folder + "tcga_genes_scae.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=mi_rna_test, fname=result_folder + "micro_rna_scae.csv", delimiter=",")
    np.savetxt(X=categorical_tissue_test, fname=result_folder + "categorical_tissue_scae.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=categorical_disease_test, fname=result_folder + "categorical_disease_scae.csv", delimiter=",", fmt='%1.3f')

    np.savetxt(X=data_pred[0], fname=result_folder + "tcga_genes_scae_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[1], fname=result_folder + "micro_rna_scae_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[2], fname=result_folder + "categorical_tissue_scae_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[3], fname=result_folder + "categorical_disease_scae_pred.csv", delimiter=",", fmt='%1.3f')
    ...
    for i in range(1, 51):
        ...
        with open(result_folder + 'dropout-CAE.txt', 'ab') as file:
    ...
    for i in range(1, 26):
        ...
        with open(result_folder + 'gaussian-CAE.txt', 'ab') as file:
            np.savetxt(file, score, delimiter=",")

    print("gaussian has just been finished")


contractive_autoencoder(local_data_folder=local_dataset_folder, local_result_folder=local_results_folder, model_specific=model_spec)
