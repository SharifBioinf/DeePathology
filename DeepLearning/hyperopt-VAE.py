...
def hyperopt_vae(data_folder, result_folder, seed=2018):
    seed = seed
    np.random.seed(seed)
    ...
    def objective(params):
        ...
        mu = Dense(params['n_z'], name="mu")(h_q)
        log_sigma = Dense(params['n_z'], name="log_sigma")(h_q)
        ...
        svae = Model(inputs, [outputs_tcga, outputs_micro_rna, outputs_cl_tissue, outputs_cl_disease])

        svae.compile(optimizer=params['optimizer'], loss=[vae_loss, "mse", "cosine_proximity", "cosine_proximity"],
                     loss_weights=[1e-3, 1e-3, 5e-1, 5e-1],
                     metrics={"m_rna": ["mae", "mse"], "mi_rna": ["mae", "mse"], "cl_tissue": "acc", "cl_disease": "acc"})

        svae.fit(m_rna_train, [m_rna_train, mi_rna_train, categorical_tissue_train, categorical_disease_train],
                 batch_size=params['batch_size'], epochs=params['nb_epochs'], verbose=2)

        score = svae.evaluate(m_rna_test, [m_rna_test, mi_rna_test, categorical_tissue_test, categorical_disease_test], verbose=0,
                              batch_size=params['batch_size'])

        print(score)

        with open(result_folder + 'hyperopt-VAE.txt', 'ab') as file:
    ...
    def run_trials(max_trials=100):
        ...
        while counter < max_trials:
            if isfile(result_folder + 'trials_svae.obj'):
                file_trials = open(result_folder + 'trials_svae.obj', 'rb')
                ...
                if len(trials.trials) < max_trials:
                    ...
                    file_trials = open(result_folder + 'trials_svae.obj', 'wb')
...
def job():
    ...
    if free_gpu_status == 0:
        ...
        hyperopt_vae(data_folder=dataset_folder, result_folder=results_folder)
        time.sleep(6)
