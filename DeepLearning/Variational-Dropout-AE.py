from keras.layers import Input, Dense, BatchNormalization, Lambda, Dropout, GaussianNoise, GaussianDropout
...
model_spec = "optimal_" # or hyperopt


def variational_dropout_autoencoder(local_data_folder, local_result_folder, model_specific, seed=2018):
    seed = seed
    ...
    def create_model():
        ...
        decoded_tcga = Dense(m_rna.shape[1], activation='linear', name="m_rna")
    ...
    file_name = "best-svae-dropout.log"
    ...
    model.save(filepath=result_folder + "svae-dropout.h5")

    layer_name = "h_q"
    encoded_layer_model = Model(inputs=model.input, outputs=model.get_layer(layer_name).output)
    encoded_output = encoded_layer_model.predict(m_rna)
    np.savetxt(X=encoded_output, fname=result_folder + "encoded_svae_dropout.csv", delimiter=",")
    ...
    np.savetxt(X=gen_data, fname=result_folder + "gen_data_vae_dropout.csv", delimiter=",", fmt='%1.3f')

    # save the result and prediction value

    data_pred = model.predict(m_rna_test, batch_size=batch_size, verbose=2)
    np.savetxt(X=m_rna_test, fname=result_folder + "tcga_genes_svae_dropout.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=mi_rna_test, fname=result_folder + "micro_rna_svae_dropout.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=categorical_tissue_test, fname=result_folder + "categorical_tissue_svae_dropout.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=categorical_disease_test, fname=result_folder + "categorical_disease_svae_dropout.csv", delimiter=",", fmt='%1.3f')

    np.savetxt(X=data_pred[0], fname=result_folder + "tcga_genes_svae_dropout_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[1], fname=result_folder + "micro_rna_svae_dropout_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[2], fname=result_folder + "categorical_tissue_svae_dropout_pred.csv", delimiter=",", fmt='%1.3f')
    np.savetxt(X=data_pred[3], fname=result_folder + "categorical_disease_svae_dropout_pred.csv", delimiter=",", fmt='%1.3f')
    ...
    for i in range(1, 51):
        ...
        with open(result_folder + 'dropout-Dropout-VAE.txt', 'ab') as file:
    ...
    for i in range(1, 51):
        ...
        with open(result_folder + 'gaussian-Dropout-VAE.txt', 'ab') as file:
            np.savetxt(file, score, delimiter=",")

    print("gaussian has just been finished")


variational_dropout_autoencoder(local_data_folder=local_dataset_folder, local_result_folder=local_results_folder, model_specific=model_spec)
