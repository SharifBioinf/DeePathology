from keras.layers import Input, Dense, BatchNormalization, GaussianNoise, GaussianDropout, multiply
...
def hyperopt_cae(data_folder, result_folder, seed=2018):
    seed = seed
    np.random.seed(seed)
    ...
    space = {
        ...
        'layer4': hp.choice('layer4', [8, 12, 16, 20, 24, 28, 32]),
    ...
    def objective(params):
        ...
        scae = Model(inputs=inputs, outputs=[decoded_tcga, decoded_micro_rna, decoded_cl_tissue, decoded_cl_disease])
        ...
        scae.compile(optimizer=params['optimizer'], loss=[contractive_loss, "mse", "cosine_proximity", "cosine_proximity"],
        ...
        score = scae.evaluate(m_rna_test, [m_rna_test, mi_rna_test, categorical_tissue_test, categorical_disease_test],
                              verbose=0, batch_size=params['batch_size'])
        print(score)

        with open(result_folder + 'hyperopt-CAE.txt', 'ab') as file:
    ...
    def run_trials(max_trials=100):
        ...
        while counter < max_trials:
            if isfile(result_folder + 'trials_scae.obj'):
                file_trials = open(result_folder + 'trials_scae.obj', 'rb')
                ...
                if len(trials.trials) < max_trials:
                    ...
                    file_trials = open(result_folder + 'trials_scae.obj', 'wb')
...
def job():
    ...
    if free_gpu_status == 0:
        ...
        hyperopt_cae(data_folder=dataset_folder, result_folder=results_folder)
        time.sleep(6)
